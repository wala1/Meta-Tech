...
public class ShowComments extends SideMenuBaseForm {
    ...
    public ShowComments(Resources res, Publication publication) {
        ...
        ArrayList<Commentaire> commentaires = ServiceComment.getInstance().showComments(publication);
        ...
        for (Commentaire co : commentaires) {
            ...
            deleteComment.addActionListener((l) -> {
                ServiceComment.getInstance().deleteComment(co.getId());
                if (ServiceComment.getInstance().deleteComment(co.getId())) {
            ...
            Label commentLabel = new Label(co.getDescription());
            commentLabel.getAllStyles().setFgColor(0xffffff);

            Label date = new Label("Date: " + co.getTemps());

            Container post = BoxLayout.encloseY(commentLabel, date);
